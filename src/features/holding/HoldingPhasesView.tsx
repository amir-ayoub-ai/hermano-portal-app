/**
 * View das fases da Holding (de Execução da Holding no ClickUp).
 *
 * Componente compartilhado pelo cliente (HoldingModule) e pelo admin
 * (ClientDetail → aba Progresso). Recebe um clientId e mostra:
 *   - Progresso geral (média ponderada)
 *   - Cards de cada fase, com status, % e sub-tarefas (expansíveis)
 */
import {
  AlertTriangle,
  CalendarClock,
...
function StatusBadge({ status }: { status: RemoteTaskStatus }) {
  const cfg: Record<
    RemoteTaskStatus,
    {
      variant: "muted" | "gold" | "success" | "warn";
      cls?: string;
      icon?: typeof Clock;
    }
  > = {
    ...
    in_review: {
      variant: "warn",
      cls: "bg-blue-500/10 border-blue-500/40 text-blue-700 dark:text-blue-400",
      icon: Eye,
    },
    ...
    blocked: {
      variant: "warn",
      cls: "bg-rose-500/10 border-rose-500/40 text-rose-700 dark:text-rose-400 font-semibold",
      icon: AlertTriangle,
    },
  };
  const c = cfg[status];
...
interface HoldingPhasesViewProps {
  clientId: string | null | undefined;
  /** Quando true, omite o card de progresso geral no topo (útil dentro de tabs do admin) */
  hideSummary?: boolean;
  /** Mensagem customizada quando não há fases */
  emptyMessage?: string;
}

export function HoldingPhasesView({
  clientId,
  hideSummary = false,
  emptyMessage,
}: HoldingPhasesViewProps) {
    ...
    return (
      <Card>
        <CardContent className="flex flex-col items-center gap-3 p-10 text-center">
          ...
          <p className="font-serif text-lg">Nenhuma fase ainda</p>
          <p className="max-w-md text-sm text-muted-foreground">
            {emptyMessage ||
              message ||
              "Assim que as tarefas de Execução da Holding forem criadas no ClickUp, elas aparecerão aqui automaticamente."}
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      {hideSummary && phases.length > 0 && (
        <div className="flex items-center justify-between rounded-lg border border-border bg-card/60 p-3 text-sm">
          <span className="text-muted-foreground">
            <strong className="text-foreground">{totalProgress}%</strong>{" "}
            concluído · {completedCount}/{phases.length} etapas
            {blockedCount > 0 && (
              <span className="ml-2 text-rose-600 dark:text-rose-400">
                · {blockedCount} travada{blockedCount > 1 ? "s" : ""}
              </span>
            )}
          </span>
        </div>
